 ...
 * @file 顧客選択モーダル（PC）
 * @description
 * 詳細画面（新規作成・編集）で「顧客選択」ボタンを追加し、
 * 顧客管理アプリから顧客を検索・選択してタスクレコードに反映する。
 *
 * - kintone のネイティブなルックアップでは要件を満たせないケースで、独自モーダルを実装するパターン
 * - 大量レコード対応: kintone API は1回最大500件 → offset でループして全件取得しキャッシュ
 * - 絞り込みはフロント側で実施（API の like 検索の仕様を回避）
 * - `kintone.app.record.getSpaceElement` でスペースフィールドにボタンを差し込む
 * - ボタンクリック時に `kintone.app.record.get/set` で現在レコードを更新する
 *
 * 前提:
 * - environments-manage プラグインの環境変数に `CUSTOMER_APP_ID`（顧客管理アプリのアプリID）を登録
 * - タスク管理アプリにスペースフィールド `customer_select_button` を配置
 * - タスク管理アプリに `customer_id` / `customer_name` の各フィールドを配置
 */
(() => {
  'use strict';
  ...
  /**
   * キーワードで顧客を絞り込む（顧客ID・顧客名のいずれかに部分一致）
   * @param {Array} records
  ...
  /**
   * 「設定」ボタン押下時のハンドラ: 現在レコードに顧客情報を反映してモーダルを閉じる
   * @param {Object} record 顧客レコード
   * @param {HTMLElement} overlay モーダルのオーバーレイ要素
   * @param {Function} onCustomerSet 反映後コールバック
   */
  const handleSetCustomer = (record, overlay, onCustomerSet) => {
    const row = kintone.app.record.get();
    row.record['customer_id'].value = record.customer_id.value;
    row.record['customer_name'].value = record.customer_name.value;
    kintone.app.record.set(row);
  ...
  const createCustomerRow = (record, overlay, onCustomerSet) => {
    ...
    setButton.style.cssText = 'padding:4px 12px;cursor:pointer;';
  ...
  const renderCustomersList = (records, list, overlay, onCustomerSet) => {
    ...
    records.forEach((record) => {
      const customerRow = createCustomerRow(record, overlay, onCustomerSet);
      list.appendChild(customerRow);
    });
  };

  /**
   * 検索ボックス入力時に呼ばれ、現キーワードで一覧を再描画する
  ...
  /**
   * 顧客選択モーダルを表示する
   * @param {Function} onCustomerSet 顧客設定後のコールバック
   */
  const showCustomerModal = (onCustomerSet) => {
    ...
    modal.style.cssText = 'background:#fff;border-radius:8px;padding:20px;max-width:700px;width:90%;max-height:80vh;overflow-y:auto;';

    const searchInput = document.createElement('input');
    searchInput.type = 'text';
    searchInput.placeholder = '顧客ID・顧客名で検索...';
    searchInput.style.cssText =
      'width:100%;padding:6px 8px;border:1px solid #ccc;border-radius:4px;box-sizing:border-box;margin-bottom:12px;';
   ...
   * @param {boolean} hasCustomer 既に顧客が設定されているか
   */
  const addCustomerButton = (hasCustomer) => {
    const space = kintone.app.record.getSpaceElement('customer_select_button');
    ...
    selectButton.style.cssText = 'padding:6px 14px;cursor:pointer;';
    selectButton.style.display = hasCustomer ? 'none' : '';

    const clearButton = document.createElement('button');
    clearButton.textContent = '顧客解除';
    clearButton.style.cssText = 'padding:6px 14px;cursor:pointer;';
